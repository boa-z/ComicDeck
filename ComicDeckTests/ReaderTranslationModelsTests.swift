...
final class ReaderTranslationModelsTests: XCTestCase {
    ...
    func testInferredReadingDirectionMatchesSourceLanguageDefaults() {
        XCTAssertEqual(
            ReaderPageTranslationBackendSupport.inferredReadingDirection(sourceLanguage: .japanese),
            .verticalRL
        )
        XCTAssertEqual(
            ReaderPageTranslationBackendSupport.inferredReadingDirection(sourceLanguage: .english),
            .horizontalLTR
        )
        XCTAssertEqual(
            ReaderPageTranslationBackendSupport.inferredReadingDirection(sourceLanguage: nil),
            .horizontalLTR
        )
    }
}
