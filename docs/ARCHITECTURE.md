...
- animated WebP/GIF/APNG/HEICS data is decoded through `AnimatedImageDecoder` with frame-count, pixel-dimension, and decoded-memory limits; shared thumbnails and both reader layouts use the same timing model, while Reduce Motion or active translation overlays fall back to the first frame
- current-page export loads the selected page through `ReaderImagePipeline`, decodes with the shared decoded-image store, renders translation overlays when the translated view is active, and performs ImageIO encoding plus file writes in a utility-priority task
- macOS reader drag-out uses `NSFilePromiseProvider`, so PNG encoding starts only when the drop destination requests the file and runs on the provider's utility operation queue instead of blocking mouse-drag handling
