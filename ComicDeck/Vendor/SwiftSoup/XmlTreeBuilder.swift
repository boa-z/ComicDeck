...
/**
 ...
 Usage example:
 ```swift
 let xmlDoc = soupParse(html, baseUrl, Parser.xmlParser())
 ```
 */
    ...
    func insert(_ commentToken: Token.Comment)throws {
        ...
        if (commentToken.bogus) { // xml declarations are emitted as bogus comments (which is right for html, but not xml)
            ...
            if (data.count > 1 && (data.startsWith("!") || data.startsWith("?"))) {
                let doc: Document = try soupParse("<" + data.substring(1, data.count - 2) + ">", String(decoding: baseUri, as: UTF8.self), Parser.xmlParser())
