...
open class Token {
    ...
    final class StartTag: Tag {
        ...
        @inline(__always)
        func resolveTag(_ settings: ParseSettings, isSelfClosing: Bool) throws -> SoupTag {
            if let cached = resolvedTag,
               resolvedTagPreservesCase == settings.preservesTagCase(),
               resolvedTagIsSelfClosing == isSelfClosing {
                return cached
            }
            let resolved: SoupTag
            if settings.preservesTagCase() {
                resolved = try SoupTag.valueOf(try name(), settings, isSelfClosing: isSelfClosing)
            } else if let fastTag = SoupTag.valueOfTagId(tagId) {
                resolved = fastTag
            } else if let normalName = normalName() {
                resolved = try SoupTag.valueOfNormalized(normalName, isSelfClosing: isSelfClosing)
            } else {
                resolved = try SoupTag.valueOf(try name(), settings, isSelfClosing: isSelfClosing)
            }
